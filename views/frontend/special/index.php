...
/* @var $projects SpecialProject[] */

$this->setPageTitle(Yii::app()->name . ' - Спецпроекты');
?>

<div class="b-main b-rubrics">
    <div class="b-news__short__list">
        <?php foreach ($projects as $sp): ?>
            <div class="b-news__short__block b-col__1">
                <a href="<?= $sp->getUrl() ?>">
                    <?php if (Yii::app()->userComponent->isAuthenticated()): ?>
                        <div class="b-news__short__faves">
                            <div data-href="#form_add_post_view" data-post_id="<?= $sp->id ?>" data-v="1" class="popup-with-form_faves addPostViewToFaves">
                                <svg class="svg_<?= $sp->id ?>" width="16" height="16" viewBox="0 0 16 16" fill="<?= $post->isFavorite() ? 'black' : 'none' ?>"
                    ...
                    <div class="b-news__short__image">
                        <img src="<?= $sp->getImageUrl('image_list') ?>" alt="" width="300" height="200">
                        <div class="b-news__short__text"><p><?= $sp->description ?></p></div>
                    </div>
                    <h2 class="b-rubrics__short_title">
                        <span><?= $sp->title ?></span>
                    </h2>
                </a>
            </div>
