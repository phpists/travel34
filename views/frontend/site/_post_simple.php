    ...
    <a href="<?= $post->getUrl() ?>">
        <?php if (Yii::app()->userComponent->isAuthenticated()): ?>
            <div class="b-news__short__faves">
                <div data-href="#form_add_post_view" data-post_id="<?= $post->id ?>" data-v="3"
                     class="popup-with-form_faves addPostViewToFaves">
        ...
        <div class="b-news__short__info">
            <?php if ($rubric = $post->getRubricTitle()): ?><span
                    class="b-news__short__category"><?= $rubric ?></span><?php endif; ?>
            <?php if (!$hideViewCount): ?><span
                    class="b-news__short__view"><?= $post->getViewsCount() ?></span><?php endif; ?>
            <?php /*<span class="b-news__short__comment"><?= $post->getCommentsCount() ?></span>*/ ?>
        </div>
        <h2 class="b-news__short__title">
            <?php if ($post->type_id == Post::TYPE_PHOTO) { ?>
                <i class="b-icon__photo__black"></i>
            <?php } elseif ($post->type_id == Post::TYPE_MINIGUIDE) { ?>
                <i class="b-icon__gide__black"></i>
