 ...
 *
 *
 * This extension is useful when you need only one attribute of selected rows from database, and loading whole
 * objects for them is not only big data pull and hense slow, but also it's more code, as you will have to
 * traverse through all the objects and foreach of them put the required attrute's value in the array, and
 * then proceed with it...
 *
 * With this extension you have one function similar to findAll, in which one extra attribute selects you
 * need to provide, and it return array containing all row's values for that attribute, or attributes..
 *
...
{
    public function findColumn($column, $condition = '', $params = [])
    {
